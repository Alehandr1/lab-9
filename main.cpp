...
{
    std::array<int, size_t(5)> arr = {10, 45, 62, 24, 88};
    ...
    std::cout << "Access element with \".at()\": " << arr.at(1) << std::endl;
    
    std::cout << "Access element with \"operator []\": " << arr[2] << std::endl;

    std::cout << "Sum of elements of array: " << sum(arr) << std::endl;

    std::cout << "Average of elements of array: " << average(arr) << std::endl;

    std::cout << "Find element \"3\" position in array: " << std::distance(arr.begin(), std::find(arr.begin(), arr.end(), 11)) << std::endl;
    
    std::replace(arr.begin(), arr.end(), 52, 99);
